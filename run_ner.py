...
flags.DEFINE_integer(
    "max_seq_length", 128, # 384 recommended #BERT can take upto 512 tokens #Could not be tried due to the GPU runtime shortage
...
class NerProcessor(DataProcessor):
    ...
    def get_labels(self):
        return ["[PAD]", "B","I", "O", "X", "[CLS]", "[SEP]"] #labels for medication events # we have three classes
    ...
    def model_fn(features, labels, mode, params):
        ...
        if mode == tf.estimator.ModeKeys.TRAIN:
            ...
            def _compute_calibration_bin_statistics(num_bins, logits=None, labels_true=None, labels_predicted=None):
                ...
                prob_y = tf.gather(
                   pred, pred_y[:, tf.newaxis], batch_dims=1) 
